...
if(isSet($_GET['usercode']) && isSet($_GET['password']))
{
	// username and password sent from Form
	$usercode= mysqli_real_escape_string($db,$_GET['usercode']); 
	...
	if(!$db->query("call pr_login_word_user(@p_userCode, @p_userPwd, @x_userId, @x_userMode, @x_returnCode, @x_returnMsg)"))
	die("CALL failed: (" . $db->errno . ") " . $db->error);
	 
	echo "CALL failed: (" . $db->errno . ") " . $db->error;
	
	// Fetch OUT parameters 
	if (!($res = $db->query("select @x_userId as user_id , @x_userMode as user_mode ,@x_returnCode as return_code, @x_returnMsg as return_msg")))
